...
class P2:
    ...
    def build(self, char_name, personality_trait):
        keywords_build_prompt, keywords = self._get_result(f"Words related to {personality_trait}? (format: Comma sperated words)")
        personality_build_prompt, personality = self._get_result(f"{keywords} are traits of {char_name}.\n\nDesribe about {char_name}", prefill=f"Here's a description of {char_name}, built from the traits suggested by the list:")
    ...
    def _get_result(self, prompt, prefill=None):
        ...
        if prefill:
            ...
            tokenized["input_ids"] = torch.cat([tokenized["input_ids"], prefill_ids], dim=1)
        ...
        output_text = self.tokenizer.decode(outputs[0, len_input:], skip_special_tokens=True)
