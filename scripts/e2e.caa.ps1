# End-to-End Test Script for PSYCTL
# This script performs a complete pipeline test: dataset build -> extract -> steering
...
uv run psyctl dataset.build.steer `
    ...
    --limit-samples 100 `
    --output "./results/test_e2e/reject_steer_dataset" 
...
Write-Host "`n=== Step 2: Extracting Steering Vector ===" -ForegroundColor Cyan
uv run psyctl extract.steering `
    --model "google/gemma-3-270m-it" `
    --layer "model.layers[13].mlp" `
    --method mean_diff `
    --dataset "./results/test_e2e/reject_steer_dataset" `
    --output "./results/test_e2e/reject_steer.safetensors"
...
uv run psyctl steering `
    --model "google/gemma-3-270m-it" `
    --steering-vector "./results/test_e2e/reject_steer.safetensors" `
