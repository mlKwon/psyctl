...
def get_hf_token():
    ...
    if not token:
        pytest.skip(
            "HF_TOKEN environment variable not set. Set it to run integration tests."
        )
    return token
...
def test_p2_real_initialization(model_and_tokenizer):
    ...
    assert (
        not hasattr(p2, "personality_build_prompt")
        or p2.personality_build_prompt is None
    )
...
def test_build_stores_all_values(p2_real_instance):
    ...
    assert (
        not hasattr(p2_real_instance, "keywords_build_prompt")
        or p2_real_instance.keywords_build_prompt is None
    )
    assert (
        not hasattr(p2_real_instance, "personality_build_prompt")
        or p2_real_instance.personality_build_prompt is None
    )
    ...
    assert (
        p2_real_instance.keywords.lower()
        in p2_real_instance.personality_build_prompt.lower()
    )
